...
\appendix
\chapter{Supplimental Methods}

\section{Spearman's Rank Correlation Calculation}
...
are valid to a degree of uncertainty.  Spearman's Rank Correlation Coefficient
(denoted by the Greek letter \textit{rho} $\rho$) is a nonparametric measure
of association between two variables.  Spearman's coefficient is an appropriate
measure for deriving association between interaction datatsets because it
assumes some monotonic relationship between variables, rather than a linear
relationship as in Pearson's correlation coefficient.  The formula for Spearman's
$\rho$ is given as follows:
...
The six replicate experiments in our 
