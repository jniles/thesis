...
\newacronym{ICE}{ICE}{Iterative Correction and Eigenvector Decompositon}
...
\newglossaryentry{trans contacts}{%
  name={trans contacts},
  description={Intractions that occur between bins on two different chromosomes}%
}

\newglossaryentry{cis contacts}{%
  name={cis contacts},
  description={Intractions that occur between bins on the same chromosome}%
}
...
\newglossaryentry{normalization}{%
  name={normalization},
  description={An attempt to compensate for noise and systematic bias from in a particular dataset, while preserving the differences in a dataset.}
}

\newglossaryentry{toric}{%
  name={toric},
  description={A background model for a distribution.}
}
