...
\newacronym[description=Gene Expression Omnibus.  Located at \url{http://www.ncbi.nlm.nih.gov/geo/}]{GEO}{GEO}{Gene Expression Omnibus}
...
\newacronym[
  ...
  description=The ordered (by associated eigenvalue) eigenvectors of the covariance matrix for some data matrix
...
\newacronym[description=An algorithm to solve the maximum likelihood estimation problem with incomplete data first by finding an estimate for the likelihood function then maximizing the whole term]
{EM}{EM}{expectation maximization}

\newacronym[description=A procedure for adjusting a table of data cells such that they add up to selected totals for both columns and rows (in the two dimensional case) of the table]
{IPF}{IPF}{iterative proportional fitting}

\newacronym[description=A technique given a set of observations and a known probability distribution of finding parameters for the underlying distribution that maximize the likelihood of observing the result]
{MLE}{MLE}{maximum likelihood estimation}

\newacronym[description=A statistical algorithm for labeling a sequence of observations from a state path and observed sequence by assuming each observation is the product of an underlying `hidden' state]
...
\newglossaryentry{mRNA}{%
  name={mRNA},
  description={messenger RNA transcribed from a DNA gene by an RNA polymerase bearing the nucleic acid code to encode a protein polypeptide}
}
...
\newglossaryentry{operon}{%
  name={operon},
  description={a unit made up of linked genes that is thought to regulate other genes responsible for protein synthesis}
}

\newglossaryentry{non-disjunction}{%
  name={non-disjunction},
  description={the failure of one or more pairs of homologous chromosomes or sister chromatids to separate normally during nuclear division, usually resulting in an abnormal distribution of chromosomes in the daughter nuclei}
}
