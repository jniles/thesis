...
them into a graph.  As $L \rightarrow \infty$, the resolution of the matrix increases, and interactions on a smller scale can be
...
interactions occuring on one string, the genome consists of 24 independent chromosome `strings'.  Furthermore, interaction maps are
...
The first step of any data analysis pipeline is data \gls{normalization}; that is, remove experimental biases and establish reproducibility.
...
probabilistic background model that computes the probability of \glspl{trans contacts} and \glspl{cis contact} based on a the regional
...
parameters on the empirically produced contact maps, which are then applied to the experimentally derived contact maps.  The mappings
...
the marginal values are the \glspl{sufficient statistic} of the model.  In other words, the maximum likelihood parameters for this data is given
...
To prove that any square matrix $\matr{A} \in M_n(\mathbb{R})$ can be decomposed into singular values, we will use some matrix definitions.
