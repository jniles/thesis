...
\begin{figure}[H]
  \centering
  \caption{Example: Interactions on a String}\label{fig:string}
...
\begin{algorithm}[H]
  ...
  Initialize $W^{0}_{ij} = \matr{O_{ij}}$; $B^0 = 1$; $k = 0$\;
...
model is called a \gls{log-linear} or \gls{toric model}.  $p$ is an observation from some distribution $p \sim f(X)$.  The challenge to
